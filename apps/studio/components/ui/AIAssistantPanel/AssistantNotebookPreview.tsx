...
export interface AssistantNotebookPreviewProps {
  ...
  title?: string
  /** Query results keyed by persisted cell id. Used by run previews only. */
      ...
      <div className="p-2">
        <div
          className={cn(
            mode === 'run'
              ? 'flex flex-col gap-2'
              : 'overflow-hidden rounded-md border bg-surface-100'
          )}
        >
          <div className={cn(mode === 'run' ? 'contents' : 'divide-y divide-border')}>
            {visibleEntries.map((entry) => {
              ...
              return (
                <div
                  key={key}
                  className={cn(
                    mode === 'run' && 'overflow-hidden rounded-md border bg-surface-100'
                  )}
                >
          ...
          {hiddenCount > 0 && (
            <Button
              ...
              className={cn(
                ...
                mode !== 'run' && 'rounded-none border-0 border-t border-default'
              )}
